...
scr's v1.1 README (just shipped in plan-cordoba-lessons Phase 6) prominently features `/planning` and `/implementation` in the workflow narrative, the worked-example quickstart, and the skills table. But those two skills aren't bundled with scr — they ship with super-claudio-code. A researcher who runs `npm install -g super-claudio-research` + `scr init` today gets brainstorming, learning-capture, verify, deliverable-review, etc., but the workflow breaks the moment they invoke `/planning`.
...
- **Don't vendor `tdd`.** Research has no TDD analog. Replace TDD cross-references with `/verify` (per-artifact sanity check, scr's existing skill).
...
- **Skill names unchanged: `planning`, `implementation`, `agent-teams`.** scr's brainstorming skill (already shipped in Phase 3 of cordoba-lessons) triggers "the planning skill" by name — `brainstorm-format.md` explicitly says "agnostic about whose". Renaming would force chasing references in already-shipped scr skills. Last-installer-wins for users with both scc and scr installed; document the precedence in README.
- **Project-identity check reads `CLAUDE.md`, not `.scc/status/project.md`.** scr's `CLAUDE.md` is already the convention-pointer + project-orientation file (seeded by `scr init`). No new hidden directory.
- **Drop `.scc/status/plan-[name].md`.** scr's per-plan `handoff.md` is the source of truth for session state. No new hidden status directory.
- **Drop the framework's `context-monitor` hook cross-reference.** scc-specific. scr has `precompact-handoff.sh` for compaction nudges; that's the analog and it's already wired.
- **Add `scr plan init <slug>` CLI subcommand.** Parity with `scc plan init`. Small lift (one new lib file + one cli.js wire-up); the planning skill instructs users to run it.
...
- **Don't break scc-resident users.** A user with both scr and scc installed will have whichever installed last winning the symlink. Document in README so the precedence is explicit.
